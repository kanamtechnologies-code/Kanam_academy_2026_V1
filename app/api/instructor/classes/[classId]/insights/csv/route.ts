...
export async function GET(
  ...
  const learners = (rows ?? [])
    ...
    .filter((x): x is { id: string; display_name: string | null; grade: string | null } =>
      Boolean(x)
    );
  ...
  try {
    ...
    const csv = classInsightsToCsv(insights);
    const safeName = String(klass.name || "class")
      .replace(/[^a-zA-Z0-9_-]+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 40);
    const filename = `kanam-class-insights-${safeName || "class"}-${classId.slice(0, 8)}.csv`;
