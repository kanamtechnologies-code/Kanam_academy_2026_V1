...
['greeted', 'quest given', 'other']`,
      ...
      },
      {
        ...
        title: "Fill the design sheet first",
        body: `The capstone scaffold has a **design sheet** at the top — comment lines where you invent your character, quest, keywords, and fallback before writing any logic.\n\nSpend two minutes there first. A clear design makes every TODO in the scaffold faster to fill in, because you already know your character's name, your three keywords, and what the else branch should say.`,
        bullets: [
          "Character name, role, and one world fact — decide before coding.",
          "Three keywords with a reply idea for each — write them as comments first.",
          "One kind else fallback — plan the confused response before you need it.",
        ],
        callout: {
          label: "Pro habit",
          text: "Design on paper (or in comments) before code. The best capstone projects start with a filled design sheet, not a blank scaffold.",
        },
      },
      {
        ...
        body: `*You've built 13 lessons of skills — variables, input, rules, loops, lists, dictionaries, functions, and NPC logic. Which two skills do you think matter most for your adventure bot, and why?*\n\nThere's no single right answer — but being able to defend your choice shows you understand how the pieces connect.`,
      },
      {
        id: "worked-example-respond",
        kicker: "Worked example",
        title: "Tracing respond() with a real message",
        body: `Let's trace one call to \`respond()\` step by step. The player types \`"I need a quest"\`, and your function checks rules in order:\n\n1. Normalize: \`player_input.lower()\` → \`"i need a quest"\`\n2. Rule 1: does \`"quest"\` appear in the message? **Yes** → print the quest reply, append to the log, **return** (stop checking).\n3. Rules 2 and 3 never run because rule 1 already matched.\n\nThat early return is why rule order matters — first match wins, everything below is skipped.`,
        code: `def respond(player_input):\n    msg = player_input.lower()\n    if "quest" in msg:\n        print("Your quest awaits in the Crystal Caves!")\n        quest_log.append("quest asked")\n    elif "help" in msg:\n        print("I can talk about quests, the world, or myself.")\n        quest_log.append("help asked")\n    else:\n        print("I'm not sure I understand. Try asking about a quest!")\n        quest_log.append("confused")`,
        codeCaption: "Trace which rule matches before you run",
      },
      {
        id: "bug-walkthrough-respond",
        kicker: "Bug walkthrough",
        title: "When respond() always hits the else branch",
        body: `If every test message — even ones with clear keywords — gets the confused fallback, work through this checklist:\n\n1. Are you calling \`.lower()\` on \`player_input\` before checking keywords?\n2. Are your keywords spelled the same way in the rules and in your test messages?\n3. Is the \`if\`/\`elif\` block indented correctly inside \`respond()\`?\n4. Are you using \`in\` (keyword appears anywhere) instead of \`==\` (exact whole message)?`,
        checkIn: {
      ...
      {
        id: "debug-habit-capstone",
        kicker: "Debugging habit",
        title: "Green checklist items tell you what's left",
        body: `The capstone checklist isn't just grading — it's a **debugging roadmap**. Each red item points to a specific missing piece:\n\n• Red on dictionary? → fill \`bot_memory\`.\n• Red on function? → write and call \`respond()\`.\n• Red on loop? → make sure Build mode replays \`test_messages\`.\n• Red on play? → open Adventure and send 3+ live messages.\n\nFix one red item at a time, run, and watch it turn green. That's the loop.`,
      },
      {
        id: "transfer-real-apps",
        kicker: "Transfer",
        title: "You built a real software architecture",
        body: `Step back and look at what your capstone actually contains:\n\n• **Memory layer** — a dictionary holding character and world data.\n• **Log layer** — a list recording every interaction.\n• **Logic layer** — a function with ordered rules.\n• **Test layer** — a fixed message list for repeatable checks.\n• **Live layer** — Adventure mode for unpredictable human input.\n\nThat's a genuine three-tier architecture — the same shape used in production chatbots, just smaller. You didn't follow a tutorial; you designed it.`,
        callout: {
          label: "Celebrate",
          text: "Finishing this capstone means you can read, write, and debug real Python — not just complete exercises.",
        },
      },
      {
        id: "check-yourself-2",
        kicker: "Check yourself",
        title: "One more design check",
        body: `Before you start coding, confirm you understand the full capstone flow.`,
        checkIn: {
