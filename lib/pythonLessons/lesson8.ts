  ...
  lessonModule: {
    ...
    sections: [
      ...
      },
      {
        id: "list-indexing",
        kicker: "Level up",
        title: "Lists have positions — starting at 0",
        body: `Every item in a list has a **position number** called an index. Python counts from **0**, not 1 — so the first item is \`choices[0]\`, the second is \`choices[1]\`, and so on.\n\nThis matters when you want to grab a specific item without searching the whole list.`,
        code: `choices = ["rock", "paper", "scissors"]\nprint(choices[0])  # first item\nprint(choices[2])  # third item`,
        codeCaption: "Index 0 is always the first item",
        output: `rock\nscissors`,
        checkIn: {
          prompt: "In `pets = [\"cat\", \"dog\", \"fish\"]`, what does `pets[1]` return?",
          choices: ["cat", "dog", "fish"],
          correctIndex: 1,
          explanation: "Index 1 is the second item — dog. Index 0 would be cat.",
        },
      },
      {
        id: "bug-walkthrough-remove",
        kicker: "Bug walkthrough",
        title: "When .remove() crashes your program",
        body: `\`.remove()\` only works if the exact item is already in the list. If you try to remove something that isn't there, Python raises a \`ValueError\` and stops.\n\nTwo safe habits:\n1. **Check first** — use \`if item in choices:\` before calling \`.remove()\`.\n2. **Match exactly** — \`.remove("Rock")\` won't find \`"rock"\`; spelling and capitalization must match.`,
        code: `choices = ["rock", "paper"]\n\n# Safe — check before removing\nif "scissors" in choices:\n    choices.remove("scissors")\nelse:\n    print("scissors wasn't in the list")`,
        codeCaption: "Check membership before removing",
        callout: {
          label: "Debugging tip",
          text: "Print the list right before .remove() to confirm the item is actually there — and spelled exactly the same way.",
        },
      },
      {
        id: "transfer-real-apps",
        kicker: "Transfer",
        title: "Lists are the backbone of collections",
        body: `Almost every app that shows a collection of things is using a list underneath:\n\n• Your **photo gallery** — a list of image files, displayed one by one.\n• A **leaderboard** — a list of scores, sorted and shown in order.\n• A **chat history** — a list of messages, newest at the bottom.\n\nThe \`.append()\` and \`.remove()\` methods you learned today are the same operations those apps use — just wrapped in prettier buttons.`,
      },
      {
