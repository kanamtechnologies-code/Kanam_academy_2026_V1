    ...
    sections: [
      {
        ...
        body: `Your helper can talk and listen. Now it learns to **decide** — to do one thing in one situation and something different in another.\n\nThis is called **rule-based decision making**, and it's exactly how many early AI systems worked: a human writes the rules, and the computer follows them *exactly*. The computer never guesses — it checks your rule and picks a path.`,
        ...
        callout: {
          ...
          text: "\"If the password is correct, log in — otherwise show an error.\" Games, apps, and spam filters are full of these if/else rules.",
        },
      },
      {
        id: "if",
        ...
        body: `An \`if\` statement asks a yes/no question. If the answer is **True**, Python runs the indented lines underneath. Don't forget the **colon** \`:\` at the end of the if line.`,
        code: `if name == "Alex":\n    print("Welcome back, Alex!")`,
      ...
      },
      {
        ...
        body: `This trips up everyone, so burn it in now:\n\n• \`=\` (one equals) means **assign** — put a value in a box: \`name = "Alex"\`.\n• \`==\` (two equals) means **compare** — ask "are these the same?": \`name == "Alex"\`.\n\nInside an \`if\`, you almost always want \`==\`.`,
        bullets: [
          "`name = \"Alex\"` → stores Alex in the box.",
      ...
      },
      {
        ...
        body: `\`else\` is the fallback. It runs when the \`if\` condition is **False**, so your program always has something to do. Only **one** path runs — never both.`,
        code: `if name == "Alex":\n    print("Welcome back, Alex!")\nelse:\n    print("Hello there!")`,
      ...
      },
      {
        ...
        body: `A good tester checks **every** path. In the exercises, run your program once with \`Alex\` (you should get the special welcome) and once with a different name (you should get the other message).\n\nClick **Start the exercises** when you're ready.`,
