  ...
  lessonModule: {
    ...
    sections: [
      ...
      },
      {
        ...
        title: "Parameter vs. argument — same idea, two moments",
        body: `These two words describe the same slot at different times:\n\n• **Parameter** — the placeholder in the \`def\` line: \`def attack(enemy):\`\n• **Argument** — the real value at call time: \`attack("goblin")\`\n\nThink of a parameter like a blank on a form, and an argument like what you actually write in that blank.`,
        code: `def greet(name):      # name is the PARAMETER\ngreet("Alex")          # "Alex" is the ARGUMENT\ngreet("Jordan")       # "Jordan" is a different ARGUMENT`,
        codeCaption: "One parameter, two different arguments",
        checkIn: {
          prompt: "In `def show_score(score):` followed by `show_score(42)`, which is the argument?",
          choices: ["score", "def", "42"],
          correctIndex: 2,
          explanation: "42 is the real value passed at call time — that's the argument. score is the parameter placeholder in the definition.",
        },
      },
      {
        id: "worked-example-2",
        kicker: "Worked example",
        title: "One function handles every enemy",
        body: `Instead of writing a separate function for each enemy, one parameterized function handles them all. Adding a new enemy type is just one more call — no new code to write.\n\nTrace what happens when you call \`attack("goblin")\` vs \`attack("dragon")\`: same function body, different argument, different output.`,
        code: `def attack(enemy):\n    print("You attack the " + enemy + "!")\n\nattack("goblin")\nattack("dragon")\nattack("slime")`,
        codeCaption: "Three enemies, one function, three calls",
        output: `You attack the goblin!\nYou attack the dragon!\nYou attack the slime!`,
      },
      {
        id: "bug-walkthrough-param",
        kicker: "Bug walkthrough",
        title: "When every call prints the same thing",
        body: `If \`attack("goblin")\` and \`attack("dragon")\` produce identical output, the bug is almost always inside the function body: the \`print\` statement is **hard-coding** a value instead of using the parameter.\n\nCompare the broken and fixed versions — the only difference is whether the print uses \`enemy\` or a fixed string.`,
        code: `# Broken — ignores the parameter\ndef attack(enemy):\n    print("You attack the goblin!")\n\n# Fixed — uses the parameter\ndef attack(enemy):\n    print("You attack the " + enemy + "!")`,
        codeCaption: "If output never changes, check whether the body uses the parameter",
      },
      {
        id: "transfer-real-apps",
        kicker: "Transfer",
        title: "APIs are functions with parameters",
        body: `When a weather app shows the forecast for your city, it calls something like \`get_forecast(city="Boston")\`. When a map app calculates a route, it calls \`get_directions(from_address, to_address)\`.\n\nEvery API endpoint you hear about is essentially a function with parameters — the app passes in real values (arguments), and the function returns a result. You're learning the same pattern the internet runs on.`,
      },
      {
