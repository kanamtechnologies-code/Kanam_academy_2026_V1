  ...
  lessonModule: {
    ...
    sections: [
      ...
      },
      {
        ...
        title: "Define vs. call — two separate moments",
        body: `A function has two distinct moments in your program:\n\n1. **Definition** (\`def greet():\`) — you write the recipe. Nothing runs yet.\n2. **Call** (\`greet()\`) — you actually cook. Python jumps to the function body and runs it.\n\nYou can define a function at the top of your file and call it many lines later — or call it five times in a row. The definition is the template; each call is a fresh execution.`,
        code: `def greet():\n    print("Hello from your helper!")\n\n# Definition above — nothing printed yet\ngreet()  # call 1 — prints\ngreet()  # call 2 — prints again`,
        codeCaption: "One definition, two calls, two lines of output",
        output: `Hello from your helper!\nHello from your helper!`,
        checkIn: {
      ...
      },
      {
        id: "worked-example-2",
        kicker: "Worked example",
        title: "One function, three calls, one change",
        body: `Here's why functions beat copy-paste: you define \`greet()\` once, then call it three times. If you want to change the greeting later, you edit **one place** — not three.\n\nThis is the core payoff of functions: write once, reuse everywhere, update in one spot.`,
        code: `def greet():\n    print("KanamBot: Ready to help!")\n\ngreet()\ngreet()\ngreet()`,
        codeCaption: "Change the message once inside def — all three calls update",
        output: `KanamBot: Ready to help!\nKanamBot: Ready to help!\nKanamBot: Ready to help!`,
      },
      {
        id: "bug-walkthrough-call",
        kicker: "Bug walkthrough",
        title: "When your function exists but never runs",
        body: `The #1 function bug for beginners: a perfect \`def\` block sitting in the file, never called. Python reads the definition, stores it, and moves on — producing zero output.\n\nQuick fix checklist:\n1. Scroll below your \`def\` — is there a \`function_name()\` line?\n2. Did you forget the parentheses? \`greet\` alone does nothing; \`greet()\` runs it.\n3. Is the call **after** the definition? Python reads top to bottom.`,
        callout: {
          label: "Debugging habit",
          text: "If your program runs with no errors and no output, your first question should be: did I actually call my function?",
        },
      },
      {
        id: "transfer-real-apps",
        kicker: "Transfer",
        title: "Every button tap is a function call",
        body: `When you tap "Send" in a messaging app, the app doesn't rewrite the send logic from scratch. It calls a \`send_message()\` function that a developer wrote once.\n\nWhen you tap "Like" on a post, same thing — \`like_post()\` runs. One definition, millions of calls across millions of users.\n\nFunctions are how real software stays manageable: complex behavior gets broken into named, reusable pieces.`,
      },
      {
