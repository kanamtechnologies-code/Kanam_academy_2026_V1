  ...
  lessonModule: {
    ...
    sections: [
      ...
      },
      {
        id: "trace-table",
        kicker: "Worked example",
        title: "Trace the state turn by turn",
        body: `When a program remembers and changes values, the fastest way to understand it is a **trace table** — write down the value before each turn, what prints, and the value after.\n\nTry tracing this loop for 3 turns. Fill in the table in your head before checking the output below.`,
        code: `message = "ping"\nfor i in range(3):\n    print(message)\n    if message == "ping":\n        message = "pong"\n    else:\n        message = "ping"`,
        codeCaption: "Trace: value before → print → value after",
        output: `ping\npong\nping`,
        bullets: [
          "Turn 1: message is ping → prints ping → flips to pong",
          "Turn 2: message is pong → prints pong → flips to ping",
          "Turn 3: message is ping → prints ping → flips to pong",
        ],
      },
      {
        id: "bug-walkthrough-state",
        kicker: "Bug walkthrough",
        title: "When the pattern repeats instead of alternating",
        body: `If your loop prints \`ping ping ping\` instead of \`ping pong ping\`, the flip logic probably isn't running — or it's running outside the loop.\n\nChecklist:\n1. Is the \`if\`/\`else\` **indented inside** the \`for\` loop?\n2. Does the flip line actually assign a **new value** to \`message\`?\n3. Are you comparing with \`==\`, not assigning with \`=\`?\n\nMost "stuck pattern" bugs come from missing indentation under the loop.`,
        checkIn: {
      ...
      },
      {
        ...
        kicker: "Transfer",
        title: "State is how apps remember you",
        body: `Every app that "knows" something about you is storing **state** — values that persist and change over time:\n\n• A game remembers your score between rounds.\n• A music app remembers what song is playing.\n• A chatbot remembers your name from earlier in the conversation.\n\nThe \`message\` variable you flipped today is a tiny version of the same idea. Professional programs just store more values and update them with more complex rules.`,
        callout: {
          label: "Preview",
          text: "In the next lessons you'll store state in lists and dictionaries — containers built for remembering many pieces of information at once.",
        },
      },
      {
